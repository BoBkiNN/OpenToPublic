...
public class PortContainer implements Serializable {
    ...
    public static void saveBackup(PortContainer container, File file){
        ...
        try {
            if (!file.getParentFile().exists()) if (!file.getParentFile().mkdirs()) throw new IOException("Failed to create folders for " + file);
    ...
    public static PortContainer loadBackup(File file){
        ...
        try {
            ...
            oos.close();
            res.fileExists = true;
            if (res.isEmpty()) if (!file.delete()) throw new IOException("Failed to delete empty file " + file);
